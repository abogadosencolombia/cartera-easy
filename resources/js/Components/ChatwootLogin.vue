...
<template>
    <div v-if="isAuthenticated" class="fixed bottom-5 right-5 z-[100]">
        <!-- Burbuja del Chat (Rediseñada) -->
        <button 
            type="button"
            @click="toggle"
            class="w-14 h-14 bg-indigo-600 rounded-full shadow-[0_10px_30px_rgba(79,70,229,0.3)] flex items-center justify-center hover:scale-110 active:scale-95 transition-all duration-300 focus:outline-none group"
            :class="{ 'rotate-90': isOpen }"
        >
            <svg v-if="!isOpen" xmlns="http://www.w3.org/2000/svg" class="h-7 w-7 text-white group-hover:-rotate-12 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor">
