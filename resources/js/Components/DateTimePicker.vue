...
// Necesario para que el componente funcione correctamente con Teleport
const { bottom, left, top } = useElementBounding(container);
const { height: windowHeight } = useWindowSize();

const floatingStyles = computed(() => {
    const dropdownHeight = 450; // Altura estimada para el selector de fecha + hora
...
<template>
    <Popover v-slot="{ open }" class="relative w-full" ref="container">
        ...
        <!-- 
            TELEPORT: Renderiza el calendario fuera del contenedor padre para evitar recortes de overflow.
            Z-INDEX: 11000 asegura visibilidad sobre modales (z-10000).
        -->
        <Teleport :to="teleport" v-if="teleport">
            <transition enter-active-class="transition duration-200 ease-out" enter-from-class="translate-y-1 opacity-0" enter-to-class="translate-y-0 opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="translate-y-0 opacity-100" leave-to-class="translate-y-1 opacity-0">
                <PopoverPanel v-slot="{ close }" class="fixed z-[11000] mt-2 left-0 w-80 origin-top-left" :style="floatingStyles">
