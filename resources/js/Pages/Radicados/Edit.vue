  ...
  <AuthenticatedLayout>
              ...
              <div class="sticky top-8 space-y-6">
                <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg p-6">
                  ...
                  <div class="border-t pt-6 space-y-6">
                      ...
                      <!-- Resto de campos laterales sin cambios -->
                      <div>
                      ...
                      <div>
                          ...
                          <AsyncSelect id="abogado" v-model="form.abogado_id" :endpoint="route('users.search')" />
                      </div>
                      <div>
                          <InputLabel for="responsable" value="Responsable de Revisión" />
                          <AsyncSelect id="responsable" v-model="form.responsable_revision_id" :endpoint="route('users.search')" />
                      </div>
                      <div>
                          <InputLabel for="correo_radicacion" value="Correo de radicación" />
                          <TextInput id="correo_radicacion" v-model="form.correo_radicacion" class="mt-1 block w-full" />
                      </div>
                      <div>
                          <InputLabel for="correos_juzgado" value="Correos Juzgado" />
                          <TextInput id="correos_juzgado" v-model="form.correos_juzgado" class="mt-1 block w-full" />
                      </div>
                      <div>
                          <InputLabel for="link_expediente" value="Link Expediente" />
                          <TextInput id="link_expediente" v-model="form.link_expediente" class="mt-1 block w-full" />
                      </div>
                      <div>
                          <InputLabel for="ubicacion_drive" value="Ubicación Drive" />
                          <TextInput id="ubicacion_drive" v-model="form.ubicacion_drive" class="mt-1 block w-full" />
                      </div>
                  </div>
                </div>
              </div>
    ...
    <Modal :show="showCloseModal" @close="showCloseModal = false">
       <div class="p-6">
         ...
         <p class="mt-1 text-sm text-gray-600">Por favor, proporciona una nota de cierre.</p>
         <div class="mt-6">
           <Textarea v-model="closeForm.nota_cierre" class="w-full" rows="4" />
         </div>
         <div class="mt-6 flex justify-end gap-3">
           <SecondaryButton @click="showCloseModal = false">Cancelar</SecondaryButton>
           <DangerButton @click="closeTheCase">Confirmar Cierre</DangerButton>
         </div>
       </div>
    </Modal>
  </AuthenticatedLayout>
