...
const selectedEtapaName = computed(() => props.etapas.find(e => e.id === form.etapa_procesal_id)?.nombre || 'Sin etapa');
...
const statusBadgeClass = computed(() => isClosed.value
...
const summaryCards = computed(() => [
    {
        label: 'Radicado',
        value: form.radicado || 'Sin radicado',
        detail: form.es_spoa_nunc ? 'Formato SPOA/NUNC' : 'Formato ordinario',
        icon: ScaleIcon,
    ...
    {
        label: 'Etapa',
        value: selectedEtapaName.value,
        detail: form.tipo_proceso_id?.nombre || 'Tipo de proceso pendiente',
        icon: CalendarDaysIcon,
        class: 'border-sky-200 bg-sky-50 text-sky-700 dark:border-sky-900/60 dark:bg-sky-950/30 dark:text-sky-300',
    },
    {
        label: 'Partes',
        value: `${form.demandantes.length} / ${form.demandados.length}`,
        detail: partyCompleteness.value.pending ? `${partyCompleteness.value.pending} registro(s) por completar` : 'Demandantes / demandados',
        icon: UsersIcon,
        class: partyCompleteness.value.pending
            ? 'border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-900/60 dark:bg-amber-950/30 dark:text-amber-300'
            : 'border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-900/60 dark:bg-emerald-950/30 dark:text-emerald-300',
    },
    {
        label: 'Juzgado',
        value: form.juzgado_id?.nombre || 'Sin juzgado',
        detail: form.correo_radicacion || 'Correo de radicación pendiente',
        icon: BuildingLibraryIcon,
        class: 'border-violet-200 bg-violet-50 text-violet-700 dark:border-violet-900/60 dark:bg-violet-950/30 dark:text-violet-300',
    },
    {
        label: 'Revisión',
        value: form.fecha_proxima_revision || 'Por definir',
        detail: form.responsable_revision_id?.name || 'Sin responsable asignado',
        icon: ShieldCheckIcon,
        class: form.fecha_proxima_revision
  ...
  <AuthenticatedLayout>
      ...
      <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div class="min-w-0">
          ...
          <div class="mt-1 flex flex-wrap items-center gap-2">
            ...
            <span class="rounded-md border px-2.5 py-1 text-[10px] font-black uppercase" :class="statusBadgeClass">
              {{ proceso.estado }}
            </span>
            <span class="rounded-md border border-indigo-200 bg-indigo-50 px-2.5 py-1 text-[10px] font-black uppercase text-indigo-700 dark:border-indigo-900/60 dark:bg-indigo-950/30 dark:text-indigo-300">
              A favor {{ form.a_favor_de === 'DEMANDANTE' ? 'demandante' : 'demandado' }}
            </span>
          </div>
          <p class="mt-1 max-w-4xl text-sm font-medium text-gray-600 dark:text-gray-300">
            Actualiza partes, juzgado, etapa, responsables, revisión y enlaces del expediente judicial.
          </p>
        </div>
        <div class="flex w-full flex-col-reverse gap-2 sm:w-auto sm:flex-row sm:items-center sm:justify-end">
          ...
          <DangerButton v-if="!isClosed" @click="openCloseModal" class="w-full justify-center sm:w-auto">Cerrar radicado</DangerButton>
        </div>
      </div>
    </template>

    <div class="radicado-edit-page min-h-screen bg-gray-50/60 py-6 dark:bg-gray-900/40">
      <div class="mx-auto max-w-[1600px] space-y-5 px-4 sm:px-6 lg:px-8">
        <div v-if="isClosed" class="flex items-start gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-900 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-100">
          <ExclamationTriangleIcon class="mt-0.5 h-5 w-5 shrink-0" />
          <p class="text-sm font-bold">Este radicado está cerrado. Los campos se muestran en modo consulta.</p>
        </div>
        ...
        <form @submit.prevent="submit" class="grid grid-cols-1 gap-5 text-gray-900 dark:text-gray-100 lg:grid-cols-[minmax(0,1fr)_24rem] xl:grid-cols-[minmax(0,1fr)_26rem]">
            ...
            <fieldset :disabled="isClosed" :class="{ 'opacity-60': isClosed }">
              <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm ring-1 ring-gray-950/5 dark:border-gray-700 dark:bg-gray-900 dark:ring-white/10">
                ...
                <div class="space-y-6 p-4 sm:p-6">
                  <div class="rounded-lg border border-indigo-100 bg-indigo-50 p-4 dark:border-indigo-800/70 dark:bg-indigo-900/15">
                    ...
                    <div class="grid max-w-xl grid-cols-2 gap-2 rounded-lg border border-indigo-100 bg-white p-1 dark:border-indigo-800 dark:bg-gray-950">
                  ...
                  <section class="space-y-4 rounded-lg border border-indigo-100 bg-indigo-50/40 p-4 dark:border-indigo-900/60 dark:bg-indigo-900/10 sm:p-5">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <button type="button" @click="addDemandante" class="inline-flex min-h-10 items-center justify-center rounded-lg bg-indigo-600 px-4 text-xs font-black text-white shadow-sm transition hover:bg-indigo-700">
                        <PlusIcon class="w-4 h-4 mr-1.5"/> Agregar otro
                      </button>
                    </div>
                    <InputError :message="form.errors.demandantes" />
                    <div v-for="(item, index) in form.demandantes" :key="index" class="relative space-y-4 rounded-lg border border-indigo-100 bg-white p-4 shadow-sm dark:border-indigo-900/50 dark:bg-gray-900 sm:p-5">
                      ...
                      <div v-else class="space-y-4 animate-in fade-in slide-in-from-top-1">
                        <label class="flex items-start gap-2 cursor-pointer rounded-lg border border-amber-200 bg-amber-50 p-3 dark:border-amber-900/60 dark:bg-amber-900/20">
                  ...
                  <section class="space-y-4 rounded-lg border border-red-100 bg-red-50/40 p-4 dark:border-red-900/60 dark:bg-red-900/10 sm:p-5">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                      ...
                      <button type="button" @click="addDemandado" class="inline-flex min-h-10 items-center justify-center rounded-lg bg-red-600 px-4 text-xs font-black text-white shadow-sm transition hover:bg-red-700">
                        <PlusIcon class="w-4 h-4 mr-1.5"/> Agregar otro
                      </button>
                    </div>
                    <InputError :message="form.errors.demandados" />
                    <div v-for="(item, index) in form.demandados" :key="index" class="relative space-y-4 rounded-lg border border-red-100 bg-white p-4 shadow-sm dark:border-red-900/50 dark:bg-gray-900 sm:p-5">
                      <div class="flex flex-col gap-3 border-b border-gray-100 pb-3 dark:border-gray-800 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <section class="grid grid-cols-1 gap-5 rounded-lg border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-800/35 md:grid-cols-2 sm:p-5">
          ...
          <aside class="min-w-0 lg:sticky lg:top-6 lg:self-start">
            <fieldset :disabled="isClosed" :class="{ 'opacity-60': isClosed }">
              <div class="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
                <div class="border-b border-gray-200 bg-gray-50/80 px-4 py-4 dark:border-gray-700 dark:bg-gray-800/60 sm:px-5">
                  <h3 class="text-base font-black text-gray-950 dark:text-gray-100">Control del radicado</h3>
                  <p class="mt-1 text-xs font-semibold text-gray-500 dark:text-gray-400">Datos de seguimiento, responsables y accesos rápidos.</p>
                </div>
                <div class="space-y-4 p-4 sm:p-5">
                  <section class="space-y-4 rounded-lg border border-indigo-100 bg-indigo-50/50 p-4 dark:border-indigo-900/60 dark:bg-indigo-950/20">
                    <div class="flex items-center gap-2 text-indigo-700 dark:text-indigo-300">
                      <ScaleIcon class="h-4 w-4" />
                      <h4 class="text-[11px] font-black uppercase tracking-widest">Identificación</h4>
                    </div>
                    <div>
                      <div class="mb-1 flex items-center justify-between gap-3">
                        <InputLabel value="Radicado" />
                        <label class="flex cursor-pointer items-center gap-1.5 text-right">
                          <input type="checkbox" v-model="form.es_spoa_nunc" class="h-3 w-3 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                          <span class="text-[9px] font-black uppercase text-gray-500 dark:text-gray-400">SPOA/NUNC</span>
                        </label>
                      </div>
                      <TextInput v-model="form.radicado" @input="handleRadicadoInput" class="mt-1 block w-full font-mono" :placeholder="form.es_spoa_nunc ? '21 dígitos (Sistema Penal)' : '14 a 23 dígitos'" />
                      <InputError :message="form.errors.radicado" />
                    </div>
                    <div>
                      <InputLabel value="Fecha de radicado" />
                      <DatePicker v-model="form.fecha_radicado" class="mt-1 block w-full" />
                      <InputError :message="form.errors.fecha_radicado" />
                    </div>
                  </section>

                  <section class="space-y-4 rounded-lg border border-gray-200 bg-gray-50/70 p-4 dark:border-gray-700 dark:bg-gray-800/40">
                    <div class="flex items-center gap-2 text-gray-700 dark:text-gray-200">
                      <ShieldCheckIcon class="h-4 w-4" />
                      <h4 class="text-[11px] font-black uppercase tracking-widest">Seguimiento</h4>
                    </div>
                    <div>
                      <InputLabel value="Abogado / Gestor" />
                      <AsyncSelect v-model="form.abogado_id" :endpoint="route('users.search')" placeholder="Asignar gestor..." label-key="name" />
                    </div>
                    <div>
                      <InputLabel value="Responsable de revisión" />
                      <AsyncSelect v-model="form.responsable_revision_id" :endpoint="route('users.search')" placeholder="Asignar responsable..." label-key="name" />
                    </div>
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
                      <div>
                        <InputLabel value="Última revisión" />
                        <DatePicker v-model="form.fecha_revision" class="mt-1 block w-full" />
                        <InputError :message="form.errors.fecha_revision" />
                      </div>
                      <div>
                        <InputLabel value="Próxima revisión" />
                        <DatePicker v-model="form.fecha_proxima_revision" class="mt-1 block w-full" />
                        <InputError :message="form.errors.fecha_proxima_revision" />
                      </div>
                    </div>
                    <div class="flex flex-wrap gap-1.5">
                      <button type="button" @click="addDays('fecha_proxima_revision', 3)" class="rounded-md bg-white px-2 py-1 text-[10px] font-black text-gray-600 ring-1 ring-gray-200 hover:bg-indigo-50 hover:text-indigo-700 dark:bg-gray-900 dark:text-gray-300 dark:ring-gray-700">+3d</button>
                      <button type="button" @click="addDays('fecha_proxima_revision', 5)" class="rounded-md bg-white px-2 py-1 text-[10px] font-black text-gray-600 ring-1 ring-gray-200 hover:bg-indigo-50 hover:text-indigo-700 dark:bg-gray-900 dark:text-gray-300 dark:ring-gray-700">+5d</button>
                      <button type="button" @click="addDays('fecha_proxima_revision', 10)" class="rounded-md bg-white px-2 py-1 text-[10px] font-black text-gray-600 ring-1 ring-gray-200 hover:bg-indigo-50 hover:text-indigo-700 dark:bg-gray-900 dark:text-gray-300 dark:ring-gray-700">+10d</button>
                      <button type="button" @click="addMonths('fecha_proxima_revision', 1)" class="rounded-md bg-white px-2 py-1 text-[10px] font-black text-gray-600 ring-1 ring-gray-200 hover:bg-indigo-50 hover:text-indigo-700 dark:bg-gray-900 dark:text-gray-300 dark:ring-gray-700">+1m</button>
                    </div>
                  </section>

                  <section class="space-y-4 rounded-lg border border-sky-100 bg-sky-50/50 p-4 dark:border-sky-900/60 dark:bg-sky-950/20">
                    <div class="flex items-center gap-2 text-sky-700 dark:text-sky-300">
                      <EnvelopeIcon class="h-4 w-4" />
                      <h4 class="text-[11px] font-black uppercase tracking-widest">Comunicaciones</h4>
                    </div>
                    <div>
                      <InputLabel value="Correo de radicación" />
                      <TextInput v-model="form.correo_radicacion" type="email" class="mt-1 block w-full" />
                      <InputError :message="form.errors.correo_radicacion" />
                    </div>
                    <div>
                      <InputLabel value="Correo(s) del juzgado" />
                      <TextInput v-model="form.correos_juzgado" class="mt-1 block w-full" placeholder="correo1@juzgado.com, correo2@..." />
                      <InputError :message="form.errors.correos_juzgado" />
                    </div>
                  </section>

                  <section class="space-y-4 rounded-lg border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
                    <div class="flex items-center gap-2 text-gray-700 dark:text-gray-200">
                      <LinkIcon class="h-4 w-4" />
                      <h4 class="text-[11px] font-black uppercase tracking-widest">Enlaces</h4>
                    </div>
                    <div>
    ...
    <Modal :show="showCommitmentModal" @close="showCommitmentModal = false" maxWidth="sm" centered>
        <div class="p-6">
            ...
            <div class="space-y-4">
                <div class="p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg border-2 border-indigo-100 dark:border-indigo-800">
