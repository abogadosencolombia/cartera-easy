...
const addDireccionRow = () => {
  form.direcciones.push(reactive({ label: '', direccion: '' }));
};
const removeDireccionRow = (idx) => {
  form.direcciones.splice(idx, 1);
};
            ...
            <form @submit.prevent="submit" class="space-y-8">
              ...
              <section>
                <div class="flex items-center justify-between mb-2">
                  ...
                  <button type="button" @click="addDireccionRow" class="inline-flex items-center gap-2 rounded-md bg-indigo-600 px-3 py-1.5 text-white text-sm hover:bg-indigo-700">
                ...
                <div v-if="!form.direcciones.length" class="rounded-md border border-dashed border-gray-300 dark:border-gray-600 p-4 text-center text-sm text-gray-500 dark:text-gray-400">
                  No hay direcciones todavía.
                </div>
                <div v-for="(d, idx) in form.direcciones" :key="idx" class="grid grid-cols-1 sm:grid-cols-12 gap-3 rounded-md border border-gray-200 dark:border-gray-700 p-3 mt-3">
                  <div class="sm:col-span-4">
                    <InputLabel :for="`dir-label-${idx}`" value="Etiqueta / Tipo" />
                    <TextInput v-model.trim="d.label" :id="`dir-label-${idx}`" type="text" class="mt-1 block w-full" list="direccion-presets" placeholder="Casa, Trabajo..." />
                  </div>
                  <div class="sm:col-span-7">
                    <InputLabel :for="`dir-direccion-${idx}`" value="Dirección Completa" />
                    <TextInput v-model.trim="d.direccion" :id="`dir-direccion-${idx}`" type="text" class="mt-1 block w-full" placeholder="Calle 123 #45-67" />
                  </div>
                  <div class="sm:col-span-1 flex items-end">
                    <button type="button" @click="removeDireccionRow(idx)" class="inline-flex w-full items-center justify-center rounded-md border border-gray-300 px-2 py-2 text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700">
                      <XMarkIcon class="h-5 w-5" />
                    </button>
                  </div>
                </div>
                <datalist id="direccion-presets">
                  <option value="Casa" />
