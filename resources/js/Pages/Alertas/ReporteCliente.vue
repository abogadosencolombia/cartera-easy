...
const props = defineProps({
  defaultDays: { type: Number, default: 15 },
});
...
const load = async () => {
  ...
  error.value   = '';
  try {
    const { data } = await axios.get('/alertas/reporte-cliente', { params: { days: days.value } });
...
const descargar = () => {
  window.location.href = `/api/alertas/reporte-cliente/export?days=${encodeURIComponent(days.value)}`;
};
...
<template>
  <Head title="Alertas - Reporte Cliente" />

  <div class="py-6">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <h1 class="text-2xl font-semibold mb-4">
        Alertas <span class="text-orange-600">Reporte cliente</span>
      </h1>
