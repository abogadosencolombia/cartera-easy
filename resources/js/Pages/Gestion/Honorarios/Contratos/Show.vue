...
const registrarPagoCargo = () => {
    pagoCargoForm.post(route('honorarios.contratos.cargos.pagar', { contrato_id: props.contrato.id }), { // Ajuste en la ruta
        preserveScroll: true,
        forceFormData: true,
...
// --- INICIO: LÓGICA PARA MODAL DE ELIMINACIÓN (Re-añadido) ---
...
// --- FIN: LÓGICA PARA MODAL DE ELIMINACIÓN ---

// --- INICIO: LÓGICA PARA FORMULARIO DE ACTUACIONES (Actualizado) ---
...
const guardarActuacion = () => {
    actuacionForm.post(route('honorarios.contratos.actuaciones.store', props.contrato.id), {
        ...
        onSuccess: () => {
            ...
            router.reload({ only: ['actuaciones'], preserveState: false }) // <-- Actualizado para refrescar datos
        },
        onError: (errors) => {
...
const eliminarActuacion = (actuacionId) => {
    if (confirm('¿Estás seguro de que quieres eliminar esta actuación? Esta acción no se puede deshacer.')) {
        ...
        router.delete(route('honorarios.contratos.actuaciones.destroy', actuacionId), {
            preserveScroll: true,
            onSuccess: () => {
                 router.reload({ only: ['actuaciones'], preserveState: false })
            },
...
const pestanaActiva = ref('cuotas') // Revertido a 'cuotas' como default
...
const navegarPagina = (url) => {
    ...
    router.get(nuevaUrl.toString(), {}, {
        preserveScroll: true,
        preserveState: true,
        only: ['cuotas', 'pagos', 'cargos', 'actuaciones', 'total_cargos_valor', 'total_pagos_valor']
    })
}
...
const abrirReestructurarModal = () => {
    ...
    if (clienteOriginal) {
        // Asumiendo que 'clientes' tiene 'id' y 'nombre_completo' (o 'nombre' si es así como viene de la prop 'clientes')
        clienteSearch.value = clienteOriginal.nombre_completo ?? clienteOriginal.nombre;
        selectedClientName.value = clienteOriginal.nombre_completo ?? clienteOriginal.nombre;
    } else {
        clienteSearch.value = '';
        ...
        <template #header>
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <template v-if="['ACTIVO','PAGOS_PENDIENTES', 'EN_MORA'].includes(props.contrato?.estado)">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div class="relative" ref="documentoMenuRef">
                                    ...
                                    <div v-if="documentoMenuAbierto" class="absolute right-0 top-full mt-1 w-48 bg-white dark:bg-gray-800 rounded-md shadow-lg border border-gray-200 dark:border-gray-700 z-10">
                                        <button @click="abrirSubirDocumentoModal(); documentoMenuAbierto = false" class="w-full text-left px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center gap-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
                                            Reemplazar Documento
                                        </button>
                                        <button @click="eliminarDocumento(); documentoMenuAbierto = false" class="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-2">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                                            Eliminar Documento
                                        </button>
                                    </div>
                            ...
                            <div class="p-4 sm:p-6">
                                <div class="border-b border-gray-200 dark:border-gray-700">
                                    <nav class="-mb-px flex space-x-4 sm:space-x-6 overflow-x-auto" aria-label="Tabs">
                                        <button @click="cambiarPestana('cuotas')" :class="[pestanaActiva === 'cuotas' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300', 'whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm']">Plan de Pagos</button>
                                        <button @click="cambiarPestana('cargos')" :class="[pestanaActiva === 'cargos' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300', 'whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm']">Cargos Adicionales</button>
                                        <button @click="cambiarPestana('pagos')" :class="[pestanaActiva === 'pagos' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300', 'whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm']">Historial de Pagos</button>
                                        <button @click="cambiarPestana('actuaciones')" :class="[pestanaActiva === 'actuaciones' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300', 'whitespace-nowrap pb-3 px-1 border-b-2 font-medium text-sm']">Actuaciones</button>
                                    </nav>
                                </div>
